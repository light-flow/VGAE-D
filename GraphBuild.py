...
class GraphBuild(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pos_weight = float(num_nodes * num_nodes - (adj.sum() + 1)) / (adj.sum() + 1)
        norm = float(num_nodes * num_nodes / ((num_nodes * num_nodes - (adj.sum() + 1)) * 2))
        ...
        weight_mask = torch.from_numpy(adj_label).view(-1) == 1
